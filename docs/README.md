# Documentation Index

| Document | Description |
|----------|-------------|
| [**Project Presentation (Faculty)**](./PROJECT_PRESENTATION.md) | Complete project explanation for faculty evaluation and viva |
| [Installation Guide](./INSTALLATION.md) | Step-by-step setup + one-command install |
| [MySQL Setup](./MYSQL_SETUP.md) | Database installation and migrations |
| [MetaMask Setup](./METAMASK_SETUP.md) | Wallet configuration for testnet |
| [Polygon Amoy Setup](./POLYGON_AMOY_SETUP.md) | Testnet deployment checklist |
| [Hardhat Deployment](./HARDHAT_DEPLOYMENT.md) | Smart contract compile & deploy |
| [API Documentation](./API.md) | REST endpoints reference |
| [Testing Guide](./TESTING.md) | Unit, integration, contract tests |

## Diagrams

| Diagram | File |
|---------|------|
| Architecture | [diagrams/ARCHITECTURE.md](./diagrams/ARCHITECTURE.md) |
| ER Diagram | [diagrams/ER_DIAGRAM.md](./diagrams/ER_DIAGRAM.md) |
| Flow Diagrams | [diagrams/FLOW_DIAGRAM.md](./diagrams/FLOW_DIAGRAM.md) |
| Sequence Diagrams | [diagrams/SEQUENCE_DIAGRAM.md](./diagrams/SEQUENCE_DIAGRAM.md) |

## Backend Deep Dives

| Document | Location |
|----------|----------|
| Database Schema | `backend/docs/DATABASE.md` |
| Blockchain Integration | `backend/docs/BLOCKCHAIN_INTEGRATION.md` |
| Security Guide | `backend/docs/SECURITY.md` |
| Contract Specification | `blockchain/docs/CONTRACT.md` |
